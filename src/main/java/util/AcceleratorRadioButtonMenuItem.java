...
/**
 * A custom radio button menu item that displays the accelerator text aligned to the right of the
 * menu item text. The purpose of this existing, rather than simply using the built-in accelerator,
 * is to circumvent the issue of the accelerator being slow when the bound key is pressed and held
 * down. This custom implementation allows the accelerator text to be displayed without any actual
 * key binding, which can instead be handled manually by listening for key pressed and released
 * events, and triggering the menu action periodically while the key is held down.
 */
public class AcceleratorRadioButtonMenuItem extends JRadioButtonMenuItem {
  ...
  public AcceleratorRadioButtonMenuItem(String text) {
    this(text, null);
  }

  public AcceleratorRadioButtonMenuItem(String text, KeyStroke keyStroke) {
    super(text);
    this.acceleratorText = keyStroke == null ? null : getKeyStrokeString(keyStroke);
    addMouseListener(
        new MouseAdapter() {
